...
full_model <- glm(cbind(deaths, confirmed - deaths) ~ ., 
                  family = binomial(), 
                  data = county_data)
```
...
tidy(full_model) %>% 
  ...
  filter(p.value < 0.05)
```
percent_hispanic: For each 1 percentage point increase in the percent of Hispanic residents in a county, the log-odds of COVID-19 death among confirmed cases decreases by 0.070, holding other variables constant.\
Converting to odds ratio: exp(-0.070) ≈ 0.933\
So, the odds of death decrease by ~6.7% for every 1% increase in the Hispanic population.\

percent_insufficient_sleep: For every 1% increase in the rate of insufficient sleep (adults who report not getting enough sleep), the log-odds of death increase by 0.092.\
Converting to odds ratio: exp(0.092) ≈ 1.096\
So, the odds of death increase by about 9.6% for every 1% increase in the rate of insufficient sleep.\

median_household_income: For each $1 increase in median household income, the log-odds of death decrease by 0.0000278.\
Convert to a meaningful unit like $10,000: 10000 * -0.0000278 = -0.278\
Converting to odds ratio: exp(-0.278) ≈ 0.757\
So, the odds of COVID-19 death decrease by about 24.3% for every $10,000 increase in median household income.\
