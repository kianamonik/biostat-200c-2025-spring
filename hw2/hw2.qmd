...
y_prop <- county_data$deaths / county_data$confirmed
wts    <- county_data$confirmed
...
# Plot the CV curve
plot(cv_lasso)
abline(v = log(c(cv_lasso$lambda.min, cv_lasso$lambda.1se)), col = c("red","blue"), lty=2)

# Extract best λ’s
lambda_min <- cv_lasso$lambda.min
lambda_1se <- cv_lasso$lambda.1se
cat("λ_min =", round(lambda_min,5), "  λ_1se =", round(lambda_1se,5), "\n\n")

# Coefficients at λ_min
coefs_min <- coef(cv_lasso, s = "lambda.min")
nz        <- which(coefs_min != 0)
selected  <- rownames(coefs_min)[nz]
cat("Nonzero coefficients at λ_min:\n")
print(selected)

# Tidy into a data.frame
lasso_coefs <- data.frame(
  term     = rownames(coefs_min)[nz],
  estimate = as.numeric(coefs_min[nz])
)
print(lasso_coefs)
```
