...
Logistic regression assumes that the log-odds of the outcome is linearly related to the predictors, that observations are independent, and that there is no multicollinearity among predictors, but our dataset may violate some of these assumptions in several ways.
First, observations (counties) may not be fully independent due to the shared state/federal-level policies or geographic proximity. Second, some predictors might have nonlinear relationships with the outcome, violating the linearity of the logit. Third, strong correlations among socioeconomic and demographic variables may lead to multicollinearity. Also, counties with very small populations or extreme CFRs may have extra undue influence on the model and lead to high-leverage or outlier effects.
...
**Solution:**
```{r}
# 1. Wald CIs via confint.default()
wald_ci <- confint.default(full_model)
colnames(wald_ci) <- c("2.5 %", "97.5 %")
print(wald_ci)

# 2. Manual: Estimate ± 1.96 * SE
coefs <- summary(full_model)$coefficients
manual_ci <- data.frame(
  Estimate = coefs[, "Estimate"],
  `2.5 %`   = coefs[, "Estimate"] - 1.96 * coefs[, "Std. Error"],
  `97.5 %`  = coefs[, "Estimate"] + 1.96 * coefs[, "Std. Error"]
)
print(manual_ci)
```
...
# 1. Profile-likelihood CIs throws out warnings
ci_profile <- confint(full_model) %>% 
  as_tibble(rownames = "term") %>% 
  rename(profile_low = `2.5 %`, profile_high = `97.5 %`)

# 2. Wald CIs (faster approximation)
...
# Prepare half‐normal quantiles & sorted leverages
n       <- length(lev)
hnq     <- qnorm((seq_len(n) - 0.5) / n)    # half‐normal quantiles
df_lev  <- data.frame(
  hnq = hnq,
  lev = sort(lev)
)
...
# Plot
ggplot(df_lev, aes(x = hnq, y = lev)) +
  geom_point(alpha = 0.6) +
  geom_abline(intercept = 0, slope = diff(range(df_lev$lev)) / diff(range(df_lev$hnq)), linetype = "dashed") +
  labs(
    title = "Half‐Normal Plot of Leverage Values",
...
step_model <- stepAIC(
  object    = full_model,
  direction = "both",
  trace     = TRUE       # shows each step’s AIC and variables dropped/added
)
...
```
Best AIC model:
cbind(deaths, confirmed - deaths) ~ state
  + percent_fair_or_poor_health
  + percent_smokers
  + percent_excessive_drinking
  + percent_uninsured
...
# 1) Build the design matrix and response
x <- model.matrix(
...
y_mat <- cbind(
  ...
  failures = county_data$confirmed - county_data$deaths
)

# 2) 10‐fold CV lasso (binomial)
set.seed(123)
cv_lasso <- cv.glmnet(
  x, y_mat,
  family       = "binomial",
...
# 3) Plot CV curve
plot(cv_lasso)
abline(v = log(c(cv_lasso$lambda.min, cv_lasso$lambda.1se)),
       col = c("red","blue"), lty = 2)

# 4) Relax the penalty: pick λ = 0.1 * λ_min
lambda <- 0.1 * cv_lasso$lambda.min
cat("Relaxed λ =", lambda, "\n")

# 5) Extract nonzero coefs at relaxed λ
coefs <- coef(cv_lasso$glmnet.fit, s = lambda)
nz_idx      <- which(coefs != 0)
data.frame(
  term     = rownames(coefs)[nz_idx],
  estimate = as.numeric(coefs[nz_idx])
)
```
